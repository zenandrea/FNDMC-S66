...
#Dmethods['QMCP_AE']   = 'QMCP_AE'
#Dmethods['QMCP_test']  = 'QMCP_ccECP_TM'
...
Dcolor['dltm5_Jopt_']  = 'orange'
Dcolor['QMCP_AE']    = 'lightgray'
Dcolor['QMCP_test']  = 'gray'
...
plt.figure()
for ref in Eb_ref:
    plt.axhline(y=Eb_ref[ref],c=sty[ref]['c'],linestyle=sty[ref]['ls'],label=f'{Eb_ref[ref]:.2f} {ref}')
#plt.xlim(0,None)
plt.xlim(0,0.103)
...
for method in methods:
        ...
        if os.path.isfile( csvfile ):
            ...
            if makefit:
                ...
                popt, pcov = curve_fit( myfun, xdata=xdata, ydata=ydata, sigma=sigma )
                xlinspace = np.linspace( 0, 0.103, 100)
                #xlinspace = np.linspace( 0, 0.053, 100)
                funlinspace = myfun( xlinspace, *popt )
                print( f'popt = {popt}' )
                perr = np.sqrt(np.diag(pcov)) 
                print( f'perr = {perr}' )
                print( f'pcov = {pcov}\n' )
                plt.plot( xlinspace, funlinspace, '--', color=Dcolor[method], label=f'{popt[0]:.2f}({perr[0]*100:.0f}) {Dmethods[method]}' )
                #plt.xlim([0,0.033])
                plt.ylim([-22,-16])
        else:
